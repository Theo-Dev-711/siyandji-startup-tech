...
const Nav = ({openNav}: Props) => {
  ...
  return (
    <div className={`transition-all ${navbg ? 'bg-white shadow-md' : 'transparent'}  duration-200 h-[12vh] z-[100] fixed w-full`}>
      <div className='flex items-center  h-full justify-between w-[90%] xl:[80%]  mx-auto '>
        {/* logo */}
        <div className='flex items-center space-x-2 '>
          <div className='w-10 h-10 rounded-full flex items-center justify-center bg-blue-800'>
              <GrTechnology className='w-6 h-6 text-white'/>
          </div>
          <h1 className='text-xl hidden sm:block font-bold md:text-2xl text-blue-800'>TechDev</h1>
        </div>
        {/* NavLinks */}
        <div className='hidden lg:flex items-center space-x-10'>
          {navLinks.map((link)=>{
          return(
            <Link href={link.url} key={link.id} className='text-black hover:text-rose-600 font-semibold duration-200 transition-all'>
              <p className=' w-fit text-[20px]'>{link.label}</p>
            </Link>
          )
        })}
